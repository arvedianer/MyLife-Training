...
const wrapper = fs.readFileSync('components/ui/SvgMaleWrapper.tsx', 'utf8');

const frontMatch = wrapper.match(/d=\\"(M 309\.48[^\\"\\\n]+)\\"/);
const backMatch = wrapper.match(/d=\\"(M 1028\.14[^\\"\\\n]+)\\"/);

const FRONT_OUTLINE = frontMatch ? frontMatch[1] : '';
const BACK_OUTLINE = backMatch ? backMatch[1] : '';
            ...
            {items.map((part) => {
                ...
                const renderPaths = (paths?: string[]) => {
                    ...
                    return paths.map((d, i) => (
                        <path key={d + i} d={d} fill={fill} stroke={BODY_STROKE} strokeWidth={1} filter={pathGlow} className={className} />
                    ));
                };
...
const FRONT_OUTLINE = "${FRONT_OUTLINE}";
const BACK_OUTLINE = "${BACK_OUTLINE}";
...
function FrontBody({ muscleSets, maxSets }: BodyHeatmapProps) {
    return (
        <svg viewBox="0 20 724 1400" width="100%" height="100%" style={{ display: \'block\', maxHeight: \'400px\' }}>
            ...
            <path d={FRONT_OUTLINE} fill={BODY_FILL} stroke={BODY_STROKE} strokeWidth={2} />
...
function BackBody({ muscleSets, maxSets }: BodyHeatmapProps) {
    return (
        <svg viewBox="724 20 724 1400" width="100%" height="100%" style={{ display: \'block\', maxHeight: \'400px\' }}>
            ...
            <path d={BACK_OUTLINE} fill={BODY_FILL} stroke={BODY_STROKE} strokeWidth={2} />
...
fs.writeFileSync('components/ui/BodyHeatmap.tsx', componentCode);
console.log('Successfully wrote BodyHeatmap.tsx');
